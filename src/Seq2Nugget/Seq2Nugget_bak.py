...
from ptnlp.layers.MaxMarginLossLayer import MaxMarginLossLayer
from ptnlp.layers.BagLossWithMarginLayer import BagLossWithMarginLayer
from ptnlp.layers.BagLossLayer import BagLossLayer
...
class Seq2Nugget(object):
    ...
    def decode_and_log(self,dev_data = None,test_data = None):
        if dev_data:
    ...
    def train_detection_epoch(self,data):
        ...
        for batch in data.mini_batches_for_train(self.batch_size):
            ...
            weight = [self.negative_weight] + [1.0] * 7
    ...
    def restore_detection_model(self,file_name):
        if os.path.exists(file_name+ ".detection_model"):
            self.detection_model.load_state_dict(torch.load(file_name+ ".detection_model"))
    ...
    def restore_boundary_model(self,file_name):
        if os.path.exists(file_name+ ".boundary_model"):
            self.boundary_model.load_state_dict(torch.load(file_name+ ".boundary_model"))
